...
**Status**: Done  
**Amends**: [v0.1.0](documentation-site-v0.1.0.md) — adds Issue Verbatim section (Issue #7, v0.3.1)
...
> **User**: imranypatel
> **Type**: `type/verbatim-cli` (auxiliary) + `type/feature` (primary)
> **Domain**: `domain/it`

---

```
What if we want make our documentation visible in user friendly documentation site or digital book form instead of making user search or navigate through folders and files in the repository?
```
...
| File | Purpose |
|------|---------|
| `docs/intro.md` | Documentation landing page (intro, section links) |
| `versions.json` | Docusaurus versions registry: `["v0.3.0", "v0.2.0", "v0.1.0"]` |
...
| `versioned_docs/version-v0.3.0/` | Snapshot of docs at merge time |
| `versioned_sidebars/version-v0.{1,2,3}.0-sidebars.json` | Per-version sidebars |
...
/docs/plans/issue-5/documentation-site-v0.1.0
```
...
|-------|-----------|---------|
| v0.3.0 (latest) | `/docs/` (default) | Documentation site release docs |
| v0.2.0 | `/docs/v0.2.0/` | ST integration docs |
| v0.1.0 | `/docs/v0.1.0/` | Governance bootstrap docs only |
| next (unreleased) | `/docs/next/` | Current branch docs (in development) |
...
| Versioned snapshots | Committed to repo | Simpler than CI-based creation; auditable in git history |
| Version creation | Manual pre-PR step | Developer runs `npm run docusaurus docs:version vX.Y.Z` before raising PR |
| `lastVersion` | `v0.3.0` | Current stable at time of release |
