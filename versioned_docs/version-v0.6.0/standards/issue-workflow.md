...
**Also amends**: [#7 — Governance: Include Issue Verbatim section in plan documents](https://github.com/imranypatel/xp-org1/issues/7) (v0.3.1)
**Status**: Active
...
Every plan file (`docs/plans/issue-N/{slug}-vX.Y.Z.md`) must follow this structure. The `## Issue Verbatim` section is **required** and must appear before `## Problem Statement`.

~~~markdown
# {Title}

**Plan Version**: vX.Y.Z
**Issue**: [#{N} — {title}](github_issue_url)
**Branch**: `{branch-name}`
...
> **Date**: {YYYY-MM-DD}
> **Type**: `type/verbatim-cli` (auxiliary) + `type/{primary}` (primary)
> **Domain**: `domain/{domain}`

---

```text
{exact verbatim text — no paraphrasing, no correction of typos}
```
...
{Agent-authored synthesis of the issue, translated into clear engineering/organizational terms.
This is the professional distillation of the verbatim above — NOT a replacement for it.}

---

## 🔄 Systems Analysis

{Required for 🔴-risk, recommended for 🟡-risk. Use the ST Analysis template above.}

---

{... remaining sections as needed: Scope, Decisions, Acceptance Criteria, etc.}
~~~

### Why Issue Verbatim is required

The verbatim text preserves the **original, unfiltered intent** that triggered the issue. The Problem Statement that follows is the agent's professional translation — inevitably shaped by interpretation, emphasis choices, and omission of ambiguity. Future readers who need to challenge or audit a decision can trace it back to the exact words, not the agent's paraphrase.

This follows the project principle: *"Remember to keep complete verbatim with identification of it as verbatim entered in a cli session."* (Issue #1 verbatim)
