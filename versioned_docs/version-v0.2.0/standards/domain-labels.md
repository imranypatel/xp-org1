...
The `st/` category provides Systems Thinking metadata. These are **optional auxiliary labels** ΓÇö they augment the existing `domain/`, `type/`, and `status/` labels but never replace them.
...
| Intervention is at a high Meadows leverage level (1ΓÇô6) | `st/leverage-point` |
