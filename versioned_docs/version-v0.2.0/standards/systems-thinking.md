...
**Systems Thinking (ST)** is the foundational analytical lens for all work in `xp-org1`. Every domain ΓÇö Operations, Finance, HR, Marketing, Infrastructure, Legal, and all others ΓÇö is analyzed through ST before interventions are designed or executed.
...
> **ST vs SD**: This standard covers *Systems Thinking* ΓÇö the qualitative, conceptual application. *Systems Dynamics* (SD) simulation (Vensim, Stella, AnyLogic) is deferred and will be addressed in a future issue when quantitative modeling is required.
...
| Net flow | Growth rate = inflow ΓêÆ outflow |

### 3. Feedback Loops

Feedback loops are the engine of system behavior. All dynamics ΓÇö growth, decay, oscillation, stability ΓÇö arise from feedback.
...
```
Example (R1 ΓÇö Governance Clarity):
Governance clarity ΓåÆ faster issue resolution ΓåÆ more issues documented
ΓåÆ richer knowledge base ΓåÆ better governance ΓåÆ [reinforces start]
```
...
```
Example (B1 ΓÇö Governance Overhead):
Governance complexity ΓåÆ overhead per issue ΓåÆ resolution slowdown
ΓåÆ pressure to simplify ΓåÆ reduced complexity ΓåÆ [balances back]
```
...
- **Oscillation** ΓÇö corrective action overshoots because feedback arrives late
- **Policy resistance** ΓÇö interventions appear not to be working, leading to abandonment
- **Counterintuitive behavior** ΓÇö the worst outcomes often appear *after* the right action was taken
...
|-----------|---------|---------|
| **Limits to Growth** | Growth pushes against a constraint that slows or reverses it | Rapid delivery rate ΓåÆ rising technical debt ΓåÆ slowing delivery |
| **Fixes that Fail** | A short-term fix creates side effects that worsen the original problem | Add headcount to fix capacity ΓåÆ coordination overhead reduces output |
...
> **Practical guidance**: Interventions at levels 1ΓÇô5 are transformational but hard to achieve. Levels 6ΓÇô9 are high-leverage and often feasible. Levels 10ΓÇô12 produce marginal results and are where most conventional management attention goes.
...
**Model**: [Link to docs/models/issue-N/ if a CLD is created ΓÇö or "Not modeled"]
```
...
- Keep diagrams focused ΓÇö one key dynamic per diagram
...
- Jay Forrester, *Industrial Dynamics* (MIT Press, 1961) ΓÇö foundational SD text
