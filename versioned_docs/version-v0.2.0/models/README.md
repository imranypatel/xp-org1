...
Every model file is linked from the corresponding GitHub Issue's **🔄 Systems Analysis** section in the Agent Assessment comment.
...
docs/models/
  README.md                          ← This file
  issue-N/
    {slug}-cld.md                   ← Causal Loop Diagram
    {slug}-sfd.md                   ← Stock-and-Flow Diagram (if needed)
    {slug}-archetype.md             ← Archetype analysis (if applicable)
    {slug}-leverage.md              ← Leverage point analysis (if applicable)
```
...
**Link notation:**
- `A -->|increases| B` — A positively influences B (same direction)
- `A -->|reduces| B` — A negatively influences B (opposite direction)
- `A -->|slows| B` — A creates a delay or friction on B
...
```markdown
# {Issue Title} — Causal Loop Diagram
...
**R1 — {Name}**: [Describe the reinforcing dynamic]
**B1 — {Name}**: [Describe the balancing dynamic]
...
| 🔴-risk issue | Create CLD at minimum |
| Multiple ST labels | One model file per type needed |
| Simple 🟢 issues | No model required |

---

## Linking Models to Issues

In the **🔄 Systems Analysis** section of the Agent Assessment comment on the GitHub issue:
